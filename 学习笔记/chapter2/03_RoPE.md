...
- [RoPE](#rope)
  - [从绝对位置编码到相对位置编码](#从绝对位置编码到相对位置编码)
    - [为什么要用位置编码](#为什么要用位置编码)
    - [绝对位置编码与其局限性](#绝对位置编码与其局限性)
    - [传统相对位置编码](#传统相对位置编码)
  - [RoPE](#rope-1)
    - [RoPE的核心设计思想](#rope的核心设计思想)
    - [为什么是旋转且匀速](#为什么是旋转且匀速)
    - [RoPE的复平面推导](#rope的复平面推导)
      - [向量与复数的映射](#向量与复数的映射)
      - [旋转矩阵的复数等价形式](#旋转矩阵的复数等价形式)
      - [复平面中的内积表示](#复平面中的内积表示)
      - [复平面中RoPE的相对性](#复平面中rope的相对性)
      - [复平面中RoPE的几何意义](#复平面中rope的几何意义)
  - [RoPE的代码实现](#rope的代码实现)
  - [旋转位置编码旋转频率的意义](#旋转位置编码旋转频率的意义)
    - [为什么不用单一旋转速度](#为什么不用单一旋转速度)
    - [旋转的高维拓展](#旋转的高维拓展)
...
绝对位置编码给每一个位置$m$分配一个唯一的向量$\bm{p}_m$，如最经典的正弦编码

$$\bm{PE}_{\left(m,2i\right)}=\sin \left(\frac{m}{10000^{\frac{2i}{d}}}\right),\quad  \bm{PE}_{\left(m,2i+1\right)}=\cos \left(\frac{m}{10000^{\frac{2i}{d}}}\right)$$

但是绝对位置编码有两个问题

+ 外推灾难

当token位置$m\le\bm{L}_{train}$即最大训练长度时，模型见过这些$m$的取值，模型表现正常

当token位置$m\gt\bm{L}_{train}$即最大训练长度时，模型面对的是分布外的输入值，根据正弦编码计算公式，正弦余弦的弧度会落入训练集未出现过的区域，这样生成的结果会偏离训练时的均值和方差
...
$$ \text{Score} = \bm{q}_m \cdot \bm{k}_n + b_{m-n} $$
...
是否存在一个函数$g$，使得$\langle \bm{Q}_m , \bm{K}_n \rangle= g\left(m-n\right)$

### 为什么是旋转且匀速

首先在2D平面上找直觉

我们的目标是，找到一个关于位置$m$的线性变换$\bm{T}_m$使得变换后的向量内积只依赖于相对距离$m-n$

设原始查询向量为$\bm{q}$，键向量为$\bm{k}$，经过位置编码后得到：

$$\bm{q}_m=\bm{T}_m\bm{q},\quad\bm{k}_n=\bm{T}_n\bm{k}$$

而我们的要求是这个变换需要满足：

$$\langle\bm{q}_m,\bm{k}_n\rangle=\langle\bm{T}_m\bm{q},\bm{T}_n\bm{k}\rangle=g\left(m-n\right)$$

将内积展开为矩阵乘法形式可得：

$$\langle\bm{T}_m\bm{q},\bm{T}_n\bm{k}\rangle=\bm{q}^T\left(\bm{T}_m^T\bm{T}_n\right)\bm{k}$$

由于上式子对任意$\bm{q},\bm{k}$都成立，而等式右边$g\left(m-n\right)$依赖于$m-n$，所以中间矩阵$\bm{T}_m^T\bm{T}_n$也依赖于$m-n$

所以必然存在一个矩阵序列$\bm{R}_{\Delta}$使得

$$\bm{T}_m^T\bm{T}_n=\bm{R}_{m-n}$$
...
$$\bm{T}_m^T\bm{T}_m=\bm{R}_0$$

而左侧向量与自身的内积数学计算结果应为模长的平方，而它的意义是变换$\bm{T}_m$对向量模长的影响

而我们的位置编码的职责是**只注入位置信息，不篡改语义强度**，所以我们要求$\bm{R}_0=\bm{I}$

这得到一个结论：任意位置$m$的位置编码变换矩阵$\bm{T}_m$满足$\bm{T}_m^T\bm{T}_m=\bm{I}$
...
当位置为0时，显然此时没有附加位置编码，$\bm{T}_0=\bm{I}$，其行列式值为1
...
那么我们假设旋转角度为$\phi\left(m\right)$，那么$\bm{T}_m$旋转了$\phi\left(m\right)$弧度

由$\bm{T}_m^T\bm{T}_n=\bm{R}_{m-n}$可知，而左侧$\bm{T}_m^T$的几何意义为$\bm{T}_m$的反向旋转操作，这个等式说明左侧只依赖于$m-n$，即
...
$$ \bm{v} = (x, y) \quad \longleftrightarrow \quad z = x + iy $$
...
$$ \bm{T}_m = \begin{pmatrix} \cos(m\theta) & -\sin(m\theta) \\ \sin(m\theta) & \cos(m\theta) \end{pmatrix} $$

将一个向量 $(x, y)$ 左乘 $\bm{T}_m$，得到旋转后的坐标 $(x', y')$：
...
在实数空间中，两个向量 $\bm{q}$ 和 $\bm{k}$ 的内积（点积）定义为：

$$ \langle \bm{q}, \bm{k} \rangle = q_x k_x + q_y k_y $$

在复数域中，如果我们设这两个向量对应的复数为 $q$ 和 $k$，则该内积等价于：

$$ \langle \bm{q}, \bm{k} \rangle = \text{Re}\big(q \cdot \overline{k}\big) $$
...
我们关心的是注意力机制中的内积 $\langle \bm{q}_m, \bm{k}_n \rangle$，它在复数域中写作：

$$ \langle \bm{q}_m, \bm{k}_n \rangle = \text{Re}\big( q_m \cdot \overline{k_n} \big) $$

将 $q_m$ 和 $k_n$ 代入：

$$ \langle \bm{q}_m, \bm{k}_n \rangle = \text{Re}\Big( \big(q \cdot e^{im\theta}\big) \cdot \overline{\big(k \cdot e^{in\theta}\big)} \Big) $$

由于共轭运算将指数取反（即 $\overline{e^{in\theta}} = e^{-in\theta}$），上式变为：

$$ \langle \bm{q}_m, \bm{k}_n \rangle = \text{Re}\Big( q \cdot e^{im\theta} \cdot \overline{k} \cdot e^{-in\theta} \Big) $$

合并指数部分：

$$ \langle \bm{q}_m, \bm{k}_n \rangle = \text{Re}\Big( q \cdot \overline{k} \cdot e^{i(m-n)\theta} \Big) \tag{1} $$

令 $S = q \cdot \overline{k}$（这是一个固定的复数，与位置无关），则最终结果为：

$$ \langle \bm{q}_m, \bm{k}_n \rangle = \text{Re}\Big( S \cdot e^{i(m-n)\theta} \Big) \tag{2} $$
...
而内积 $\langle \bm{q}_m, \bm{k}_n \rangle$ 计算的是这个夹角的余弦值，因此它天然地携带了相对距离 $(m-n)$ 的信息。
